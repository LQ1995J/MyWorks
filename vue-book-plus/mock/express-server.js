let http = require('http');
let fs = require('fs');
let url = require('url');
let express = require('express');  // express是一个函数
// app 为监听函数
let app = express();
app.listen(3030);  // 在3030 端口上开启服务
let sliders = require('./sliders'); //引入sliders.js文件
let bodyParser = require('body-parser');  //解析post数据需要用到body-parser
app.use(bodyParser.json());
app.use(express.static(__dirname));

/*获取轮播图 /sliders*/


function read(cb) {  //用来读取数据的
  fs.readFile('./book.json', 'utf8', function (err, data) {
    if (err || data.length == 0) {
      cb([]);  // 如果有错误，或者文件没长度，就是空数组
    } else {
      cb(JSON.parse(data));  //将读出来的内容转化为对象
    }
  })
}

function write(data, cb) {  //写入内容，传入的data将覆盖book.json,成功后调用回调函数cb
  fs.writeFile('./book.json', JSON.stringify(data), cb)
}

let pageSize = 5; //每页显示5个

app.use(function (req, res, next) {
  //解决跨域问题
  res.header("Access-Control-Allow-Origin", "*");
  res.header("Access-Control-Allow-Headers", "Content-Type,Content-Length, Authorization, Accept,X-Requested-With");
  res.header("Access-Control-Allow-Methods", "PUT,POST,GET,DELETE,OPTIONS");
  res.header("X-Powered-By", '3.2.1');
  if (req.method == "OPTIONS") res.sendStatus(200);
  else next();
});
app.get('/', function (req, res) {
  res.sendFile('./index.html', {root: __dirname});

});

app.get('/page', function (req, res) {  // req：代表的是请求 res：代表的是响应
  let {query} = url.parse(req.url, true);  //true把query转化成对象
  let offset = parseInt(query.offset) || 0;  //所有的路径（query.offset）都是字符串  //拿到当前前端传递的值
  read(function (books) {
    // 每次偏移量 在偏移的基础上增加五条
    let result = books.reverse().slice(offset, offset + pageSize); //数据倒序
    let hasMore = true;  //默认有更多
    if (books.length <= offset + pageSize) {  //已经显示的数目大于了总共条数
      hasMore = false;
    }
    res.json({hasMore, books: result});
  });
});

app.get('/sliders', function (req, res) {
  res.json(sliders);
});

app.get('/hot', function (req, res) {  //热门图书 一个路径对应一个接口，每个路径返回不同的结果，这就是路由
  read(function (books) {
    let hot = books.reverse().slice(0, 6);
    res.json(hot);
  });
});

app.get('/book', function (req, res) {
  let id = parseInt(req.query.id);
  if (!isNaN(id)) {   // 查询一个
    read(function (books) {
      let book = books.find(item => item.bookId === id);
      if (!book) {
        book = {};
      }   //如果没找到book，则book为undefined，则令他为空
      res.json(book);
    });
  } else { //获取所有图书
    read(function (books) {
      res.json(books.reverse());
    })
  }
});

app.post('/book', function (req, res) {
  let book = req.body;
  read(function (books) {  //添加id
    book.bookId = books.length ? books[books.length - 1].bookId + 1 : 1;
    books.push(book);
    write(books, function () {  // 将数据放到内存中的books中
      res.json(book);
    });
  });
});


app.delete('/book', function (req, res) {
  let {query} = url.parse(req.url, true);  //true把query转化成对象
  let id = parseInt(query.id);   //取出的是字符串，对象里的id，使用parseInt可转化成数值类型
  read(function (books) {
    books = books.filter(item => item.bookId !== id);
    write(books, function () {
      res.json({});  //删除返回空对象
    });
  });
});

app.put('/book', function (req, res) {
  let {query} = url.parse(req.url, true);  //true把query转化成对象
  let id = parseInt(query.id);   //取出的是字符串，对象里的id，使用parseInt可转化成数值类型
  if (req.query.id) { // 获取了当前的要修改的id
    let book = req.body;
    read(function (books) {
      books = books.map(item => {
        if (item.bookId === id) {  //找到id相同的那一本书，用新的覆盖掉老的
          return book
        }
        return item;  // 其他书正常返回即可
      });
      write(books, function () {  //将数据写回JSON中  //将所有书写入成功后会调用回调函数
        res.json(book);
      })
    });
  }
});

app.all('*', function (req, res) {
  res.readFile('./index.html', {root: __dirname})
});
